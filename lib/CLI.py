...
def main():
    ...
    config.read('/opt/apps/osg-xsede-amie/etc/osg-xsede-amie.conf')
    amie = AMIE(config)
    connect = OSGConnect(config)

    parser = argparse.ArgumentParser(prog='osg-xsede-amie')
    ...
    parser_pcreated = subparsers.add_parser('project-created',
                                            help='Use this when a XSEDE requested project and PI has been set up in OSGConnect')
    ...
    parser_acreated = subparsers.add_parser('account-created',
                                            help='Use this when a XSEDE requested user account been set up in OSGConnect')
    parser_acreated.add_argument('--global-id', required=True, help='XSEDE Global ID')
